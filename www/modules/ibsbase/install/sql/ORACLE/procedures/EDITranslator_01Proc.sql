 ...
 *
 * @version     2.21.0001, 21.03.2002 KR
 *
 * @author      Klaus Reimüller (KR) 020321
 ******************************************************************************
 */
 ...
 */
CREATE OR REPLACE FUNCTION p_EDITranslator_01$create
(
...
AS
    ...
    c_NOOID                 CONSTANT RAW (8) := createOid (0, 0);
...
*/
        ...
        ibs_error.log_error (ibs_error.error, 'p_EDITranslator_01$create', l_eText);
/* no transactions allowed because already defined in p_Translator_01$create
 ...
 *                          database.
 */
CREATE OR REPLACE FUNCTION p_EDITranslator_01$change
(
    -- common input parameters:
    ai_oid_s                VARCHAR2,
    ai_userId               INTEGER,
    ...
    ai_filterFile           VARCHAR2,
    ai_formatFile           VARCHAR2
)
RETURN INTEGER
AS
    -- constants:
...
*/
        ...
        l_eText := l_ePos ||
            ...
            ', ai_filterFile = ' || ai_filterFile ||
            ', ai_formatFile = ' || ai_formatFile ||
            '; errorcode = ' || SQLCODE ||
            ', errormessage = ' || SQLERRM;
        ibs_error.log_error (ibs_error.error, 'p_EDITranslator_01$change', l_eText);
/* no transactions allowed because already defined in p_Translator_01$change
        -- set new transaction starting point:
        COMMIT WORK;
*/
 ...
 *                          database.
 */
CREATE OR REPLACE FUNCTION p_EDITranslator_01$retrieve
(
    -- common input parameters:
    ai_oid_s                VARCHAR2,
    ai_userId               INTEGER,
    ...
    ao_filterFile           OUT VARCHAR2,
    ao_formatFile           OUT VARCHAR2
)
RETURN INTEGER
AS
    -- constants:
...
*/
        ...
        l_eText := l_ePos ||
            ...
            ', ao_filterFile = ' || ao_filterFile ||
            ', ao_formatFile = ' || ao_formatFile ||
            '; errorcode = ' || SQLCODE ||
            ', errormessage = ' || SQLERRM;
        ibs_error.log_error (ibs_error.error, 'p_EDITranslator_01$retrieve', l_eText);
/* no transactions allowed because already defined in p_Translator_01$retrieve
        -- set new transaction starting point:
        COMMIT WORK;
*/
        -- return error code:
        RETURN c_NOT_OK; 
END p_EDITranslator_01$retrieve;
/
 ...
 */
CREATE OR REPLACE FUNCTION p_EDITranslator_01$BOCopy
(
    -- common input parameters:
    ai_oid                  RAW,
    ai_userId               INTEGER,
...
*/
    ...
    THEN
        BEGIN
            ...
            SELECT  ai_newOid, filterFile, formatFile
            FROM    ibs_EDITranslator_01
            WHERE   oid = ai_oid;
        EXCEPTION
            WHEN OTHERS THEN            -- any exception
                -- create error entry:
                l_ePos := 'insert new edi translator data';
                RAISE;                  -- call common exception handler
        END;
     END IF; -- if operation properly performed
...
*/
        ...
        l_eText := l_ePos ||
            ...
            '; errorcode = ' || SQLCODE ||
            ', errormessage = ' || SQLERRM;
        ibs_error.log_error (ibs_error.error, 'p_EDITranslator_01$BOCopy', l_eText);
/* no transactions allowed because already defined in p_Translator_01$BOCopy
        -- set new transaction starting point:
        COMMIT WORK;
*/
