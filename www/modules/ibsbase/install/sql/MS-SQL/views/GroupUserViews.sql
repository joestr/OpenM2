...
IF EXISTS ( SELECT  * 
            FROM    sysobjects 
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.v_GroupUser$getAll')
                AND sysstat & 0xf = 2)
	DROP VIEW #CONFVAR.ibsbase.dbOwner#.v_GroupUser$getAll
GO

CREATE VIEW v_GroupUser$getAll
AS
    SELECT DISTINCT v.oid AS oid, v.state AS state, v.name AS name, v.userId, 
            ...
            N' ' AS ownerName, N'0000000000000000' AS ownerOid, N' ' AS ownerFullname, 
