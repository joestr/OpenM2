/******************************************************************************
 * All stored procedures regarding the Locale_01 Object. <BR>
 *
...
/******************************************************************************
 * Creates a new Locale_01 Object (incl. rights check). <BR>
 *
 * @input parameters:
...
EXEC p_dropProc N'p_Locale_01$create'
GO

-- create the new procedure:
CREATE PROCEDURE p_Locale_01$create
(
    -- input parameters:
    @ai_userId              USERID,
    @ai_op                  INT,
...
-- body:
    ...
    BEGIN TRANSACTION
        ...
        BEGIN
            ...
            INSERT INTO ibs_Locale_01 (oid, language, country, isDefault)
            SELECT  @l_oid, '', '', 0
 ...
 * @param   ai_language         The language
 * @param   ai_country          The country
 * @param   ai_isDefault        Is this the default layout
 *
 * @output parameters:
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_Locale_01$change'
GO

-- create the new procedure:
CREATE PROCEDURE p_Locale_01$change
(
    -- input parameters:
    ...
    @showInNews     BOOL,
    @ai_language    NVARCHAR (2),
    @ai_country     NVARCHAR (2),
    @ai_isDefault   BOOL
)
AS
    -- definitions:
    ...
    BEGIN TRANSACTION
        ...
        UPDATE  ibs_Locale_01
        SET     language = @ai_language,
                country = @ai_country,
 ...
 *                              the checkOut-User
 * @param   ao_language         The language
 * @param   ao_country          The country
 * @param   ao_isDefault        Is this the default layout
 *
 *
 * @returns A value representing the state of the procedure.
 *  ALL_RIGHT               Action performed, values returned, everything ok.
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_Locale_01$retrieve'
GO

-- create the new procedure:
CREATE PROCEDURE p_Locale_01$retrieve
(
    -- input parameters:
    @oid_s          OBJECTIDSTRING,
    @userId         USERID,
    ...
    @ao_language    NVARCHAR (2)    OUTPUT,
    @ao_country     NVARCHAR (2)    OUTPUT,
...
AS
    ...
    BEGIN TRANSACTION
        ...
        SELECT 
            @ao_language = language,
            @ao_country = country,
            @ao_isDefault = isDefault
        FROM    ibs_Locale_01 
        WHERE   oid = @oid
    ...
    RETURN  @retValue
GO
-- p_Locale_01$retrieve


/******************************************************************************
 * Deletes a Locale_01 object and all its values (incl. rights check). <BR>
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_Locale_01$delete'
GO

-- create the new procedure:
CREATE PROCEDURE p_Locale_01$delete
(
    @oid_s          OBJECTIDSTRING,
    @userId         USERID,
    @op             INT
...
-- delete existing procedure:
EXEC p_dropProc N'p_Locale_01$BOCopy'
GO

-- create the new procedure:
CREATE PROCEDURE p_Locale_01$BOCopy
(
...
AS
    ...
    INSERT INTO ibs_Locale_01 
            (oid, language, country, isDefault)
    SELECT  @newOid, b.language, b.country, b.isDefault
    FROM    ibs_Locale_01 b
    WHERE   b.oid = @oid
