 ...
 *
 * @version     $Id: U24006p_RightsProc.sql,v 1.3 2005/02/24 17:05:12 klaus Exp $
 *
...
DECLARE
    ...
    @l_msg                  VARCHAR (255),  -- message for printout
    @l_i                    ID,             -- loop counter
...
*/
    ...
    IF (@l_rKeysId = -1)                -- no corresponding key found?
    BEGIN
 ...
 */
-- delete existing procedure:
EXEC p_dropProc 'p_Rights$getKeyForOwner'
GO
...
-- body:
    ...
    BEGIN
        ...
        SELECT  @ao_id = rk.id
        FROM    ibs_RightsKey rk, ibs_RightsKey actrk, ibs_RightsKeys rks
        WHERE   actrk.id = @ai_actId
            AND actrk.rKeysId = rks.id
            AND rk.rKeysId = rks.id
        ...
        BEGIN
            ...
            INSERT INTO ibs_RightsKey
                    (rKeysId, owner, cnt)
            SELECT  rks.id, @l_owner, actrk.cnt
            FROM    ibs_RightsKey actrk, ibs_RightsKeys rks
            WHERE   actrk.id = @ai_actId
                AND actrk.rKeysId = rks.id
...
DECLARE
    ...
    @l_newRKey              INT,            -- id of the new rights key
    @l_owner                USERID          -- the owner of the objectsss
    ...
    BEGIN
        ...
        BEGIN
            ...
            ELSE                    -- no error
                ...
                -- object with the same old key:
                EXEC @l_retValue =
                    p_Rights$setKeyRec @ai_oid, @l_oldRKey, @l_newRKey
        END -- if
        -- get next tuple:
        FETCH NEXT FROM updateCursor INTO @l_oldRKey, @l_owner
    END -- while another tuple found
