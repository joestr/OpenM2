 ...
 *
 * @version     $Id: U24004q_RightsProc.sql,v 1.2 2005/01/26 21:27:35 klaus Exp $
 *
...
DECLARE
    ...
    @c_STATE_DELETED        INT,            -- the object was deleted
    @c_STATE_ACTIVE         INT,            -- active state of object
/* KR Performance tuning: use hard coded values
...
/* KR performance tuning:
   Each part of the query executed separately is faster than executing all at
   once.
...
... performance tuning */
    ...
    -- set rights key of the tabs of the actual object:
    UPDATE  ibs_Object
    SET     rKey = @ai_rKey
    WHERE   containerId = @ai_rOid
        AND containerKind = 2
--! HACK ...
        -- don't propagate rights to some specific object types:
        AND tVersionId NOT IN (0x01012001, 0x01010031)
/* KR Performance tuning: use hard coded values
        AND tVersionId <> @c_ParticipantContainer
        AND tVersionId <> @c_Reference
*/
--! ... HACK
    ...
    -- set rights key of links to the actual object:
    UPDATE  ibs_Object
    SET     rKey = @ai_rKey
    WHERE   linkedObjectId = @ai_rOid
        AND state = 2

/* KR 20050126 links to tab objects seem to be not existing
    -- set rights key of links to tabs of the actual object:
    UPDATE  ibs_Object
    SET     rKey = @ai_rKey
...
... performance tuning */
        ...
        -- set rights key of links to actual object and its subsequent objects
        -- having the same old rights key:
        UPDATE  ibs_Object
        SET     rKey = @ai_rKey
        WHERE   linkedObjectId IN
