...
EXEC p_dropProc N'p_User_01$performCreate'
GO
...
-- body:
    ...
    BEGIN TRANSACTION
        ...
        BEGIN
            ...
            FROM    ibs_Operation
            WHERE   name IN (N'view', N'read', N'new', N'addElem')
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$create'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$change'
GO
...
AS
    ...
    BEGIN TRANSACTION
        ...
        SELECT  @given = COUNT (*) 
        FROM    ibs_User u INNER JOIN ibs_Object o ON u.oid = o.oid
        WHERE   o.name = @name
        ...
        BEGIN
            ...
            BEGIN
                -- store the previous name of the user for the update of the
                -- workspace objects in the key mapper table:
                SELECT  @l_oldName = u.name, @l_userId = u.id
                FROM    ibs_user u
                WHERE   u.oid = @oid
                ...
                -- update of key mapper only necessary when name changed
                IF (@l_oldName <> @name)
                BEGIN
                    -- update key mapper entries of workspace objects 
                    -- Change EXTKEY from previous user name to new user name: 
                    UPDATE  ibs_KeyMapper
                    SET     id = REPLACE(id, '_' + @l_oldName, '_' + @name)   
                    WHERE   oid IN
                            (
                                SELECT  o.oid
                                FROM    ibs_Object oWsp
	                                    INNER JOIN ibs_Workspace wsp
	                                        ON oWsp.oid = wsp.workspace
	                                        AND wsp.userId = @l_userId
	                                    INNER JOIN ibs_Object o
	                                        ON o.posNoPath LIKE oWsp.posNoPath + '%'
                            )
                        AND id LIKE '%[_]' + @l_oldName + '%'
                END -- if update of key mapper only necessary when name changed
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$new'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$retrieve'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$login'
GO
...
AS
    ...
    DECLARE @realPassword NAME,
            @validUntil DATETIME
    -- initialize local variables:
    SELECT  @realPassword = N'unknownPassword',
            @oid = 0x0000000000000000, @id = 0x00000000, @fullname = N''
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$logout'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$changePassword'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$delete'
GO
...
-- body:
    ...
    BEGIN
        ...
        -- check if there occurred an error:
        EXEC @l_error = ibs_error.prepareError @@error,
            N'get workspace data', @l_ePos OUTPUT
        IF (@l_error <> 0)                  -- an error occurred?
            GOTO NonTransactionException    -- call exception handler
        ...
        BEGIN TRANSACTION -- begin new TRANSACTION
            ...
            BEGIN
                ...
                -- check if there occurred an error:
                EXEC @l_error = ibs_error.prepareError @@error,
                    N'update user state', @l_ePos OUTPUT
                IF (@l_error <> 0)      -- an error occurred?
                    GOTO exception      -- call exception handler
...
NonTransactionException:                -- error outside of transaction occurred
    -- log the error:
    EXEC ibs_error.logError 500, N'p_User_01$delete', @l_error, @l_ePos,
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$BOCopy'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$delUnneededGrNoCum'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$addNeededGrNoCum'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$setGroups'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$delUserGroups'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_User_01$getInfo'
GO
