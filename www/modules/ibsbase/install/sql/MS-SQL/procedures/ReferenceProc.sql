...
EXEC p_dropProc N'p_Reference$create'
GO
...
-- body:
    ...
    EXEC @l_error = ibs_error.prepareErrorCount @@error, @@ROWCOUNT,
        N'update existing reference', @l_ePos OUTPUT, @l_rowCount OUTPUT
    ...
    BEGIN
        ...
        EXEC @l_error = ibs_error.prepareError @@error,
            N'update existing reference', @l_ePos OUTPUT
        IF (@l_error <> 0)              -- an error occurred?
            GOTO exception              -- call common exception handler
...
exception:                              -- an error occurred
    ...
    -- log the error:
    EXEC ibs_error.logError 500, N'p_Reference$create', @l_error, @l_ePos,
            N'ai_kind', @ai_kind,
            N'ai_fieldName', @ai_fieldName
GO
-- p_Reference$create
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_Reference$delete'
GO
...
-- body:
    ...
    -- check if there occurred an error:
    EXEC @l_error = ibs_error.prepareErrorCount @@error, @@ROWCOUNT,
        N'delete existing reference', @l_ePos OUTPUT, @l_rowCount OUTPUT
...
exception:                              -- an error occurred
    ...
    -- log the error:
    EXEC ibs_error.logError 500, N'p_Reference$delete', @l_error, @l_ePos,
            N'', N'',
            N'ai_fieldName', @ai_fieldName
GO
-- p_Reference$delete
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_Reference$deleteRefcingOid'
GO
...
-- body:
    ...
    -- check if there occurred an error:
    EXEC @l_error = ibs_error.prepareError @@error,
        N'delete all references from object', @l_ePos OUTPUT
    IF (@l_error <> 0)                  -- an error occurred?
        GOTO exception                  -- call common exception handler
...
exception:                              -- an error occurred
    ...
    -- log the error:
    EXEC ibs_error.logError 500, N'p_Reference$deleteRefcingOid', @l_error, @l_ePos
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_Reference$deleteRefcedOid'
GO
...
-- body:
    ...
    -- check if there occurred an error:
    EXEC @l_error = ibs_error.prepareError @@error,
        N'delete all references to object', @l_ePos OUTPUT
    IF (@l_error <> 0)                  -- an error occurred?
        GOTO exception                  -- call common exception handler
...
exception:                              -- an error occurred
    ...
    -- log the error:
    EXEC ibs_error.logError 500, N'p_Reference$deleteRefcedOid', @l_error, @l_ePos
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc N'p_Reference$deleteTVersion'
GO
...
-- body:
    ...
    -- check if there occurred an error:
    EXEC @l_error = ibs_error.prepareError @@error,
        N'delete all references from this type''s objects', @l_ePos OUTPUT
    IF (@l_error <> 0)                  -- an error occurred?
        GOTO exception                  -- call common exception handler
    ...
    -- check if there occurred an error:
    EXEC @l_error = ibs_error.prepareError @@error,
        N'delete all references to this type''s objects', @l_ePos OUTPUT
    IF (@l_error <> 0)                  -- an error occurred?
        GOTO exception                  -- call common exception handler
...
exception:                              -- an error occurred
    ...
    -- log the error:
    EXEC ibs_error.logError 500, N'p_Reference$deleteTVersion', @l_error, @l_ePos,
