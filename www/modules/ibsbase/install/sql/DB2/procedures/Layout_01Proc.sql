...
CALL IBSDEV1.p_dropProc ('p_Layout_01$create');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_Layout_01$create
(
    -- input parameters:
    IN  ai_userId           INT,
    IN  ai_op               INT,
...
BEGIN 
    ...
    DECLARE l_linkedObjectId CHAR (8) FOR BIT DATA;       -- oid of linked object
    DECLARE l_allGroupId    INT;            -- group of all users
    DECLARE l_structAdminGroupId INT;       -- group of structure administrators
    DECLARE l_rights        INT;            -- the actual permissions
    DECLARE l_domainId      INT;            -- id of the actual domain
    DECLARE l_sysAdmin      INT;            -- user id of system administrator
    ...
    SET l_domainId          = ((ai_userId - MOD(ai_userId,16777216))/16777216);
    SET l_sysAdmin          = 0;
...
-- body:
    ...
    THEN 
        ...
        SET l_sqlcode = 0;
        INSERT  INTO IBSDEV1.ibs_Layout_01 (oid, name, domainId)
        ...
        THEN  
            ...
            THEN
                -- create error entry:
                SET l_ePos = 'Error in getting groups';
                GOTO exception1;            -- call common exception handler
            END IF; -- if any exception
            ...
            THEN 
                ...
                THEN
                    -- create error entry:
                    SET l_ePos = 'Error in SELECT SUM (id) group found';
                    GOTO exception1;    -- call common exception handler
                END IF; -- if any exception

                SET l_sqlcode = 0;
                CALL IBSDEV1.p_Rights$setRights (l_oid, l_structAdminGroupId,
                    l_rights, 1);
                ...
                THEN
                    -- create error entry:
                    SET l_ePos = 'Error in p_Rights$setRights group found';
                    GOTO exception1;    -- call common exception handler
                END IF; -- if any exception

            END IF; -- group found?
            -- set permissions for group of all users:
            IF NOT (l_allGroupId IS NULL)
            THEN 
                SET l_sqlcode = 0;
                SELECT  SUM (id) 
                INTO    l_rights
                ...
                THEN
                    -- create error entry:
                    SET l_ePos = 'Error in SELECT SUM (id) group of all';
                    GOTO exception1;    -- call common exception handler
                END IF; -- if any exception

                SET l_sqlcode = 0;
                CALL IBSDEV1.p_Rights$setRights (l_oid, l_allGroupId,
                    l_rights, 1);
                ...
                THEN
                    -- create error entry:
                    SET l_ePos = 'Error in p_Rights$setRights group of all';
                    GOTO exception1;    -- call common exception handler
                END IF; -- if any exception
...
exception1:
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_Layout_01$create',
        l_sqlcode, l_ePos,
...
--------------------------------------------------------------------------------
-- delete existing procedure:
CALL IBSDEV1.p_dropProc ('p_Layout_01$change');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_Layout_01$change
(
    -- input parameters:
...
exception1:
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_Layout_01$change',
        l_sqlcode, l_ePos,
...
--------------------------------------------------------------------------------
-- delete existing procedure:
CALL IBSDEV1.p_dropProc ('p_Layout_01$retrieve');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_Layout_01$retrieve
(
    -- input parameters:
    IN  ai_oid_s            VARCHAR (18),
    IN  ai_userId           INT,
...
exception1:
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_Layout_01$retrieve',
        l_sqlcode, l_ePos,
        'ai_userId', ai_userId, 'ai_oid_s', ai_oid_s,
        'ai_op', ai_op, '', '',
        '', 0, '', '',
...
--------------------------------------------------------------------------------
-- delete existing procedure:
CALL IBSDEV1.p_dropProc ('p_Layout_01$delete');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_Layout_01$delete
(
    IN  ai_oid_s            VARCHAR (18),
    IN  ai_userId           INT,
    IN  ai_op               INT
...
-- body:
    ...
    THEN 
        ...
        SET l_sqlcode = 0;
        DELETE  FROM IBSDEV1.ibs_Layout_01
        WHERE   oid = l_oid;
        ...
        THEN
            -- create error entry:
            SET l_ePos = 'Error in delete object type specific data';
            GOTO exception1;            -- call common exception handler
        END IF; -- if any exception
...
exception1:
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_Layout_01$delete',
        l_sqlcode, l_ePos,
        'ai_userId', ai_userId, 'ai_oid_s', ai_oid_s,
        'ai_op', ai_op, '', '',
        '', 0, '', '',
...
-- delete existing procedure:
CALL IBSDEV1.p_dropProc ('p_Layout_01$BOCopy');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_Layout_01$BOCopy
(
...
-- body:
    ...
    SET l_sqlcode = 0;
    INSERT  INTO IBSDEV1.ibs_Layout_01 (oid, name)
    SELECT  ai_newOid, b.name
    FROM    IBSDEV1.ibs_Layout_01 b
    WHERE   b.oid = ai_oid;
...
exception1:
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_Layout_01$BOCopy',
        l_sqlcode, l_ePos,
        'ai_userId', ai_userId, '', '',
        '', 0, '', '',
        '', 0, '', '',
