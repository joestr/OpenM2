...
CALL IBSDEV1.p_dropProc ('p_EDITranslator_01$create');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_EDITranslator_01$create
(
...
*/
    ...
    -- create base object:
    CALL IBSDEV1.p_Translator_01$create (ai_userId, ai_op, ai_tVersionId,
    ...
    THEN
        ...
        INSERT INTO IBSDEV1.ibs_EDITranslator_01
...
--------------------------------------------------------------------------------
-- delete existing procedure:
CALL IBSDEV1.p_dropProc ('p_EDITranslator_01$change');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_EDITranslator_01$change(
    -- common input parameters:
    ...
    IN  ai_filterFile       VARCHAR (255),
    IN  ai_formatFile       VARCHAR (255)
    )
DYNAMIC RESULT SETS 1 
LANGUAGE SQL 
BEGIN 
...
--------------------------------------------------------------------------------
-- delete existing procedure:
CALL IBSDEV1.p_dropProc ('p_EDITranslator_01$retrieve');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_EDITranslator_01$retrieve(
    -- input parameters:
    IN  ai_oid_s            VARCHAR (18),
    IN  ai_userId           INT,
    IN  ai_op               INT,
    ...
    OUT ao_filterFile       VARCHAR (255),
    OUT ao_formatFile       VARCHAR (255)
    )
DYNAMIC RESULT SETS 1 
LANGUAGE SQL 
BEGIN 
...
*/
    ...
    CALL IBSDEV1.p_Translator_01$retrieve(ai_oid_s, ai_userId, ai_op, ao_state,
        ...
        ao_checkOutUserOid, ao_checkOutUserName, ao_isMaster,
        ao_attachmentType, ao_filename, ao_path, ao_url, ao_filesize,
        ao_isWeblink, ao_extension);
    GET DIAGNOSTICS l_retValue = RETURN_STATUS;
  
    IF l_retValue = c_ALL_RIGHT THEN 
        -- retrieve the type specific data:
        SELECT filterFile, formatFile
        INTO ao_filterFile, ao_formatFile
        FROM IBSDEV1.ibs_EDITranslator_01
        WHERE oid = l_oid;
...
--------------------------------------------------------------------------------
-- delete existing procedure:
CALL IBSDEV1.p_dropProc ('p_EDITranslator_01$BOCopy');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_EDITranslator_01$BOCopy(
-- create the new procedure:
...
BEGIN 
    DECLARE SQLCODE         INT;
    -- constants:
    DECLARE c_NOT_OK        INT;            -- create the new procedure:
    DECLARE c_ALL_RIGHT     INT;            -- everything was o.k.
...
*/
    ...
    IF l_retValue = c_ALL_RIGHT THEN 
        ...
        IF ( l_sqlcode <> 0 AND l_sqlcode <> 100 ) OR l_rowcount <> 1 THEN -- an error occurred?
...
*/
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_EDITranslator_01$BOCopy', l_sqlcode, l_ePos,
