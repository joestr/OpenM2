...
CALL IBSDEV1.p_dropProc ('p_ReceivedObject_01$create');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_ReceivedObject_01$create
(
    -- input parameters:
    IN  ai_userId           INT,
    IN  ai_op               INT,
...
-- body:
    ...
    THEN 
        ...
        VALUES  (l_oid, c_NOOID, 0,null, 'no defined', 'icon.gif',
                'no defined', c_NOOID, 'no defined');
    END IF;
...
exception1:
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_ReceivedObject_01$create',
        l_sqlcode, l_ePos,
...
--------------------------------------------------------------------------------
-- create the new procedure:
CALL IBSDEV1.p_dropProc ('p_ReceivedObject_01$change');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_ReceivedObject_01$change
(
    -- input parameters:
    ...
    -- output parameters:
    OUT ai_distributedId_s  VARCHAR (18),
    OUT ai_distributedTVersionId INT,
    OUT ai_distributedTypeName VARCHAR (63),
    OUT ai_distributedName  VARCHAR (63),
    OUT ai_distributedIcon  VARCHAR (63),
    OUT ai_activities       VARCHAR (63),
    OUT ai_sentObjectId_s   VARCHAR (18)
)
DYNAMIC RESULT SETS 1 
LANGUAGE SQL 
BEGIN 
    ...
    DECLARE l_senderUid     INT;
    DECLARE l_recipientContainerId_s VARCHAR (18);
    DECLARE l_oid           CHAR (8) FOR BIT DATA;
...
-- body:
    ...
    CALL IBSDEV1.p_stringToByte (ai_sentObjectId_s, l_sentObjectId);
    CALL IBSDEV1.p_stringToByte (ai_distributedId_s, l_distributedId);

    -- check if there occurred an error:
    IF (l_sqlcode <> 0)                 -- any exception?
    THEN
        -- create error entry:
        SET l_ePos = 'Error in conversions';
        GOTO exception1;                -- call common exception handler
    END IF; -- if any exception
    ...
    THEN 
        SET l_sqlcode = 0;
        UPDATE  IBSDEV1.ibs_ReceivedObject_01
        SET     distributedId = l_distributedId,
                distributedTVersionId = ai_distributedTVersionId,
                distributedName = ai_distributedName,
                distributedIcon = ai_distributedIcon,
                sentobjectId = l_sentobjectId
        WHERE   oid = l_oid;
...
exception1:
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_ReceivedObject_01$change',
        l_sqlcode, l_ePos,
...
--------------------------------------------------------------------------------
-- delete existing procedure:
CALL IBSDEV1.p_dropProc ('p_ReceivedObject_01$retrieve');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_ReceivedObject_01$retrieve
(
    -- input parameters:
    IN  ai_oid_s            VARCHAR (18),
    IN  ai_userId           INT,
    ...
    OUT ao_distributedId_s  VARCHAR (18),
    OUT ao_distributedTVersionId INT,
    OUT ao_distributedTypeName VARCHAR (63),
    OUT ao_distributedName  VARCHAR (63),
    OUT ao_distributedIcon  VARCHAR (63),
    OUT ao_activities       VARCHAR (63),
    OUT ao_recipientContainerId_s VARCHAR (18),
    OUT ao_senderFullName   VARCHAR (63)
)
DYNAMIC RESULT SETS 1 
LANGUAGE SQL 
BEGIN 
    ...
    DECLARE l_sentObjectId  CHAR (8) FOR BIT DATA;
    DECLARE l_distributedId CHAR (8) FOR BIT DATA;
    DECLARE l_recipientId   CHAR (8) FOR BIT DATA;
    DECLARE l_sqlcode       INT DEFAULT 0;
    DECLARE l_ePos          VARCHAR (2000); -- error position description
    ...
    SET l_sentObjectId      = c_NOOID;
    SET l_distributedId     = c_NOOID;
    SET l_recipientId       = c_NOOID;
...
-- body:
    ...
    IF (l_retValue = c_ALL_RIGHT)
    THEN 
        SELECT  distributedId, distributedTVersionId, distributedTypeName,
                distributedName, distributedIcon, activities, sentObjectId,
                senderFullName
        INTO    l_distributedId, ao_distributedTVersionId,
                ao_distributedTypeName, ao_distributedName, ao_distributedIcon,
                ao_activities, l_sentObjectId, ao_senderFullName
        FROM    IBSDEV1.ibs_ReceivedObject_01
        WHERE oid = l_oid;
        ...
        WHERE   containerId = l_sentObjectId
            AND tVersionId = 16849665;

        -- convert sendedObjectId to output
        CALL IBSDEV1.p_byteToString (l_distributedId, ao_distributedId_s);
    
        -- convert sendedObjectId to output
        CALL IBSDEV1.p_byteToString (l_recipientContainerId, ao_recipientContainerId_s);
    END IF;
...
exception1:
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_ReceivedObject_01$retrieve',
        l_sqlcode, l_ePos,
        'ai_userId', ai_userId, 'ai_oid_s', ai_oid_s,
        'ai_op', ai_op, '', '',
        '', 0, '', '',
...
--------------------------------------------------------------------------------
-- delete existing procedure:
CALL IBSDEV1.p_dropProc ('p_ReceivedObject_01$delete');

-- create the new procedure:
CREATE PROCEDURE IBSDEV1.p_ReceivedObject_01$delete
(
    IN  ai_oid_s            VARCHAR (18),
    IN  ai_userId           INT,
    IN  ai_op               INT
...
-- body:
    ...
    THEN 
        ...
        DELETE FROM IBSDEV1.ibs_ReceivedObject_01
        WHERE   oid = l_oid;
    END IF;
...
exception1:
    -- log the error:
    CALL IBSDEV1.logError (500, 'p_ReceivedObject_01$delete',
        l_sqlcode, l_ePos,
        'ai_userId', ai_userId, 'ai_oid_s', ai_oid_s,
        'ai_op', ai_op, '', '',
        '', 0, '', '',
