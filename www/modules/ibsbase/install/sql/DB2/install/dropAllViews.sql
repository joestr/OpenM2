-------------------------------------------------------------------------------
-- Drop all views within the framework. <BR>
--
-- @version     $Id: dropAllViews.sql,v 1.5 2003/10/21 22:37:48 klaus Exp $
--
...
DROP PROCEDURE IBSDEV1.pi_dropAllViews;
-- create new procedure:
CREATE PROCEDURE IBSDEV1.pi_dropAllViews ()
...
BEGIN
    ...
    DECLARE object_Cursor INSENSITIVE CURSOR FOR 
        SELECT  s.name 
        FROM    IBSDEV1.sysviews s
        WHERE   s.system_view_schema = 'IBSDEV1'
            AND UPPER (s.NAME) LIKE UPPER ('v_%')
        ORDER BY s.name ASC;
...
-- body:
    ...
    DO
        ...
        SET l_exec = 'DROP VIEW IBSDEV1.' || l_name;
