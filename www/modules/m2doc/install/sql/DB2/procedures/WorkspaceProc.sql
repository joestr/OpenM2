...
--
-- @version     $Id: WorkspaceProc.sql,v 1.7 2003/10/31 00:12:52 klaus Exp $
--
...
-- body:
    ...
    CALL IBSDEV1.p_ObjectDesc_01$get
        (c_languageId, 'OD_wspShoppingCart', l_name, l_desc);
    CALL IBSDEV1.p_Object$performCreate (ai_userId, ai_op, 16847873, l_name,
        ai_workspace_s, 1, 0, c_NOOID_s, l_desc,
        l_shoppingCart_s, l_shoppingCart);
    GET DIAGNOSTICS l_retValue = RETURN_STATUS;

    IF (l_sqlcode <> 0 OR l_retValue <> c_ALL_RIGHT)
    THEN
        -- create error entry:
        SET l_ePos = 'ceate shopping cart';
        GOTO exception1;                -- call common exception handler
    END IF;

    -- orders (0x01011201 - OrderContainer)
    CALL IBSDEV1.p_ObjectDesc_01$get
        (c_languageId, 'OD_wspOrders', l_name, l_desc);
    CALL IBSDEV1.p_Object$performCreate (ai_userId, ai_op, 16847361, l_name,
        ai_workspace_s, 1, 0, c_NOOID_s, l_desc, l_orders_s, l_orders);
    GET DIAGNOSTICS l_retValue = RETURN_STATUS;

    IF (l_sqlcode <> 0 OR l_retValue <> c_ALL_RIGHT)
    THEN
        -- create error entry:
        SET l_ePos = 'ceate orders';
        GOTO exception1;                -- call common exception handler
    END IF;
