/******************************************************************************
 * All stored procedures regarding the m2_ProductBrand_01 table. <BR>
 * 
 * @version     $Id: ProductBrand_01Proc.sql,v 1.7 2003/10/31 00:13:14 klaus Exp $
 *
...
CREATE OR REPLACE FUNCTION p_ProductBrand_01$create
(
    -- input parameters:
    ai_userid           INTEGER,
    ai_op               INTEGER,
...
BEGIN
    ...
    THEN
	    ...
	    EXCEPTION
	        WHEN OTHERS THEN
	            ibs_error.log_error(ibs_error.error, 'p_ProductBrand_01$create','Error INSERT INTO');
                RAISE;
        END;
    END IF;
...
EXCEPTION
  WHEN OTHERS THEN
    ibs_error.log_error ( ibs_error.error, 'p_ProductBrand_01$create',
                          ', userid: ' || ai_userid ||
                          ', op: ' || ai_op ||
...
-- create the new procedure:
CREATE OR REPLACE FUNCTION p_ProductBrand_01$change
(
    ai_oid_s          VARCHAR2,
    ai_userid            INTEGER,
    ai_op             INTEGER,
...
EXCEPTION
  WHEN OTHERS THEN
    ibs_error.log_error ( ibs_error.error, 'p_ProductBrand_01$change',
                          ', oid_s: ' || ai_oid_s ||
                          ', userid: ' || ai_userid ||
                          ', op: ' || ai_op ||
                          ', name: ' || ai_name ||
 ...
 */
-- create the new procedure:
CREATE OR REPLACE FUNCTION p_ProductBrand_01$retrieve
(
    ai_oid_s            VARCHAR2,
    ai_userid           INTEGER,
    ai_op               INTEGER,
    ...
    THEN
	    ...
	    EXCEPTION
	        WHEN NO_DATA_FOUND THEN
	            ibs_error.log_error(ibs_error.error, 'p_ProductBrand_01$retrieve','Error SELECT');
	        RAISE;	        
	    END;
    END IF;-- if operation properly performed
...
EXCEPTION
  WHEN OTHERS THEN
    ibs_error.log_error ( ibs_error.error, 'p_ProductBrand_01$retrieve',
                          ', oid_s: ' || ai_oid_s ||
                          ', userid: ' || ai_userid ||
                          ', op: ' || ai_op ||
                          ', sqlcode: ' || sqlcode ||
...
END p_ProductBrand_01$retrieve;
/
