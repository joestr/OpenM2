 ...
 *
 * @version     $Id: ProductProfile_01Proc.sql,v 1.7 2009/12/02 18:35:03 rburgermann Exp $
 *
...
IF EXISTS (
            SELECT  *
            FROM    sysobjects
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$create')
                AND sysstat & 0xf = 4
          )
    DROP PROCEDURE #CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$create
GO

-- create the new procedure:
CREATE PROCEDURE p_ProductProfile_01$create
(
...
IF EXISTS ( SELECT  *
            FROM    sysobjects
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$change')
                AND sysstat & 0xf = 4)
    DROP PROCEDURE #CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$change
GO

-- create the new procedure:
CREATE PROCEDURE p_ProductProfile_01$change
(
    -- common input parameters:
...
AS
    ...
    SELECT  @retValue = @NOT_OK
    -- define local variables:
    DECLARE @oid OBJECTID, @begin INT, @inc INT, @oidstring NVARCHAR(20),
            @catoid OBJECTID  
    -- initialize local variables:
...
/*
        ...
        BEGIN
            SELECT @begin = 1
            SELECT @oidstring = SUBSTRING(@categories, @begin, 18) 
            WHILE @oidstring IS NOT NULL
            BEGIN
                EXEC p_StringtoByte @oidstring, @catoid
                INSERT INTO m2_ProfileCategory_01 (productProfileOid, categoryOid)
                VALUES (@oid, @catoid)
                SELECT @begin = @begin + 19
                SELECT @oidstring = SUBSTRING(@categories, @begin, 18) 
            END -- WHILE
...
IF EXISTS ( SELECT  *
            FROM    sysobjects
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$retrieve')
                AND sysstat & 0xf = 4)
    DROP PROCEDURE #CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$retrieve
GO

-- create the new procedure:
CREATE PROCEDURE p_ProductProfile_01$retrieve
(
    -- common input parameters:
    @oid_s          OBJECTIDSTRING,
    @userId         USERID,
...
IF EXISTS ( SELECT  *
            FROM    sysobjects
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$delete')
                AND sysstat & 0xf = 4)
    DROP PROCEDURE #CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$delete
GO

CREATE PROCEDURE p_ProductProfile_01$delete
(
    -- common input parameters:
    @oid_s          OBJECTIDSTRING,
    @userId         USERID,
...
IF EXISTS ( SELECT  *
            FROM    sysobjects
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$BOCopy')
                AND sysstat & 0xf = 4)
    DROP PROCEDURE #CONFVAR.ibsbase.dbOwner#.p_ProductProfile_01$BOCopy
GO

-- create the new procedure:
CREATE PROCEDURE p_ProductProfile_01$BOCopy
(
    -- common input parameters:
    @oid            OBJECTID,
    @userId         USERID,
