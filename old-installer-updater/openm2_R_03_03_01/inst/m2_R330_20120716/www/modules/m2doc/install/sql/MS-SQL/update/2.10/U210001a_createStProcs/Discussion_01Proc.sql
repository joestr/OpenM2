 ...
 *
 * @version     $Id: Discussion_01Proc.sql,v 1.1 2010/02/25 13:54:18 btatzmann Exp $
 *
...
IF EXISTS (
            SELECT  * 
            FROM    sysobjects 
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.p_Discussion_01$create') 
                AND sysstat & 0xf = 4
          )
    DROP PROCEDURE #CONFVAR.ibsbase.dbOwner#.p_Discussion_01$create
GO

-- create the new procedure:
CREATE PROCEDURE p_Discussion_01$create
(
    -- input parameters:
    @userId         USERID,
    @op             INT,
...
IF EXISTS ( SELECT  * 
            FROM    sysobjects 
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.p_Discussion_01$change') 
                AND sysstat & 0xf = 4)
    DROP PROCEDURE #CONFVAR.ibsbase.dbOwner#.p_Discussion_01$change
GO

-- create the new procedure:
CREATE PROCEDURE p_Discussion_01$change
(
    -- input parameters:
...
IF EXISTS ( SELECT  * 
            FROM    sysobjects 
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.p_Discussion_01$retrieve') 
                AND sysstat & 0xf = 4)
    DROP PROCEDURE #CONFVAR.ibsbase.dbOwner#.p_Discussion_01$retrieve
GO

-- create the new procedure:
CREATE PROCEDURE p_Discussion_01$retrieve
(
    -- input parameters:
    @oid_s          OBJECTIDSTRING,
    @userId         USERID,
...
IF EXISTS ( SELECT  * 
            FROM    sysobjects 
            WHERE   id = object_id ('#CONFVAR.ibsbase.dbOwner#.p_Discussion_01$delete') 
                AND sysstat & 0xf = 4)
    DROP PROCEDURE #CONFVAR.ibsbase.dbOwner#.p_Discussion_01$delete
GO

CREATE PROCEDURE p_Discussion_01$delete
(
    @oid_s          OBJECTIDSTRING,
    @userId         USERID,
    @op             INT
...
*/        
    ...
    RETURN  @retValue
GO
-- p_Discussion_01$delete




if exists (select * from sysobjects where id = object_id('#CONFVAR.ibsbase.dbOwner#.p_Discussion_01$BOCopy') and sysstat & 0xf = 4)
    drop procedure #CONFVAR.ibsbase.dbOwner#.p_Discussion_01$BOCopy
GO


-- create the new procedure:
CREATE PROCEDURE p_Discussion_01$BOCopy
(
...
AS
    ...
    INSERT INTO m2_Discussion_01 
            (oid, maxlevels, defaultView, refOid)
    SELECT  @newOid, b.maxlevels, b.defaultView, b.refOid
