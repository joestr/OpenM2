...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <!--*********************** VARIABLES BEGIN ****************************-->
    <!-- Defines columns not to display. Can be declard within stylesheets using
         this stylesheets. -->
    <xsl:variable name="disabledColumns"/>
    
    <xsl:variable name="showName" select="true ()"/>
          ...
          <xsl:for-each select="OBJECT">
            ...
            <INPUT TYPE="HIDDEN" NAME="" VALUE="">
				<xsl:attribute name="NAME">
				     <xsl:value-of select="SYSTEM/NAME/@INPUT"/>
				</xsl:attribute>
                <xsl:attribute name="VALUE">
                     <xsl:value-of select="SYSTEM/NAME/child::text()"/>
                </xsl:attribute>
            </INPUT>
    ...
    <xsl:template name="showHeader">
        ...
        <xsl:for-each select="/OBJECTS/OBJECT[position()=1]/VALUES/VALUE">
            ...
            <xsl:if test="not(contains ($disabledColumns, concat ('|', @FIELD, '|')))">
	            <TD CLASS="listheader">
	                <!-- ordering can not be used at the moment for columns other than
	                name since the headings array on the server is filled with the
	                headings defined within the container's document template,
	                while within content edit mode all columns can be displayed -->
