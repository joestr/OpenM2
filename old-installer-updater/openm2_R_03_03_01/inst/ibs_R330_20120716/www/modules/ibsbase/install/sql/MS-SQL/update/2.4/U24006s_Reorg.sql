 ...
 *
 *
 * @version     $Id: U24006s_Reorg.sql,v 1.2 2005/03/09 14:53:12 klaus Exp $
 *
...
SELECT
    @l_dbname = DB_NAME (),
    @l_archiveTable = @ai_table + '_archive',
    @l_error = 0,
    @l_rowCount = 0

-- body:
    -- get the full table name:
    EXEC p_getFullName @l_archiveTable, @l_archiveTable OUTPUT

    -- check if the table does not exist yet:
    IF NOT EXISTS (
        SELECT  * 
        FROM    sysobjects 
        WHERE   id = object_id (@l_archiveTable)
            AND sysstat & 0xf = 3)
    BEGIN
        SELECT  @l_message =
                '    Creating archive table ' + @l_archiveTable + '...'
--        EXEC p_showMessage @l_message, 0, 1
        ...
        '    INTO    ' + @l_archiveTable +
        '    FROM    ' + @ai_table +
        '    WHERE   COL_NAME (object_id (''' + @ai_table + '''), 1) > ' +
        '            (SELECT MAX (COL_NAME (object_id (''' + @ai_table + '''), 1)) ' +
        '            FROM ' + @ai_table + ')'
        )
...
-- body:
    ...
    EXEC p_reorgCreateTable @ai_table
    ...
    WHERE   o.type = 'U'
        ...
        AND o.name NOT IN
            (
                ...
                -- ibs_Object:
                'ibs_Object', 'ibs_Copy', 'ibs_Protocol_01', 'ibs_Reference',
                'ibs_KeyMapper', 'ibs_KeyMapperArchive',
                -- help:
                'ibs_Help_01',
                -- user management:
...
-- body:
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                -- create reorg table:
                EXEC p_reorgCreateTable @l_name
            END -- if
...
-- body:
    ...
    EXEC p_reorgCreateTable 'ibs_RightsKey'
    EXEC p_reorgCreateTable 'ibs_RightsKeys'
    EXEC p_reorgCreateTable 'ibs_RightsCum'
    ...
    BEGIN
        BEGIN TRANSACTION
            ...
            INSERT  INTO ibs_RightsKey_archive
            SELECT  *
            FROM    ibs_RightsKey
            WHERE   id >= @l_i
                AND id < (@l_i + @ai_step)
            ...
            WHERE   id IN (SELECT id FROM ibs_RightsKey_archive)
        COMMIT TRANSACTION
        ...
        BEGIN TRANSACTION
            -- put not used tuples into the archive table:
            INSERT  INTO ibs_RightsKeys_archive
            SELECT  *
            FROM    ibs_RightsKeys
            WHERE   id >= @l_i
                AND id < (@l_i + @ai_step)
                AND id NOT IN ( SELECT  rk.rKeysId
                                FROM    ibs_Object o, ibs_RightsKey rk 
                                WHERE   rk.rKey >= @l_i
                                    AND rk.rKey < (@l_i + @ai_step)
                                    AND rk.id = o.rKey)
                AND id < @l_maxId

            -- delete the moved tuples:
            DELETE  ibs_RightsKeys
            WHERE   id IN (SELECT id FROM ibs_RightsKeys_archive)
        COMMIT TRANSACTION
        ...
        BEGIN TRANSACTION
            -- put not used tuples into the archive table:
            INSERT  INTO ibs_RightsCum_archive
            SELECT  *
            FROM    ibs_RightsCum
            WHERE   rkey >= @l_i
                AND rkey < (@l_i + @ai_step)
...
DECLARE wspCursor CURSOR FOR 
    ...
    FROM    ibs_Workspace w,
        (SELECT * FROM ibs_Object UNION ALL SELECT * FROM ibs_Object_archive) o
...
-- body:
    ...
    -- ensure that the archive table exists:
    EXEC p_reorgCreateTable 'ibs_Object'
    ...
    BEGIN TRANSACTION
        -- put not used tuples into the archive table:
        INSERT  INTO ibs_Object_archive
        SELECT  *
        FROM    ibs_Object
        WHERE   state <> 2
        ...
        -- delete the moved tuples:
        DELETE  ibs_Object
        WHERE   oid IN (SELECT oid FROM ibs_Object_archive)
