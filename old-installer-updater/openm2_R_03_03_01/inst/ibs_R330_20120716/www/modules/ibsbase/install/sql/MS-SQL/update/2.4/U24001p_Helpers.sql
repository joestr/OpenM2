...
DECLARE
    -- local variables:
    @l_text                 VARCHAR (255)   -- output text
...
-- body:
    ...
    IF (@@error <> 0)                    -- no error?
    BEGIN
        SELECT  @l_text = 'Error when deleting procedure p_getFullName.'
...
(
    ...
    @ai_name                VARCHAR (255),
    -- output parameters:
    @ao_fullName            VARCHAR (255) OUTPUT
)
AS
-- body:
    -- set the full name:
    ...
    BEGIN
        ...
        SELECT  @ao_fullName = u.name + '.' + @ai_name
...
-- body:
    ...
    IF (@@error <> 0)                    -- no error?
    BEGIN
        SELECT  @l_text = 'Error when deleting procedure p_dropProc.'
        PRINT @l_text                       -- print message
    END
...
DECLARE
    -- local variables:
    @l_fullProcName         VARCHAR (255),  -- full procedure name incl. db user
    @l_text                 VARCHAR (255)   -- output text
...
-- body:
    ...
    IF (@@error <> 0)                    -- no error?
    BEGIN
        SELECT  @l_text = 'Error when deleting procedure ' + @ai_procName + '.'
        PRINT @l_text                       -- print message
    END
GO
-- p_dropProc
 ...
 */
-- delete existing procedure:
EXEC p_dropProc 'p_dropTable'
GO

-- create the new procedure:
CREATE PROCEDURE p_dropTable
(
    @ai_tableName            VARCHAR(255)
)
AS
DECLARE
    -- local variables:
    @l_fullTableName        VARCHAR (255),  -- full table name incl. db user
    @l_text                 VARCHAR (255)   -- output text
...
-- body:
    ...
    IF (@@error <> 0)                    -- no error?
    BEGIN
        SELECT  @l_text = 'Error when deleting table ' + @ai_tableName + '.'
        PRINT @l_text                       -- print message
    END        
GO
-- p_dropTable
...
DECLARE
    -- local variables:
    @l_fullViewName         VARCHAR (255),  -- full table name incl. db user
    @l_text                 VARCHAR (255)   -- output text
...
-- body:
    ...
    IF (@@error <> 0)                    -- no error?
    BEGIN
        SELECT  @l_text = 'Error when deleting view ' + @ai_viewName + '.'
        PRINT @l_text                       -- print message
    END
GO
-- p_dropView
...
DECLARE
    -- local variables:
    @l_fullTrigName         VARCHAR (255),  -- full trigger name incl. db user
    @l_text                 VARCHAR (255)   -- output text
...
-- body:
    ...
    IF (@@error <> 0)                    -- no error?
    BEGIN
        SELECT  @l_text = 'Error when deleting trigger ' + @ai_trigName + '.'
        PRINT @l_text                       -- print message
    END
GO
-- p_dropTrig
...
DECLARE
    -- local variables:
    @l_fullFuncName         VARCHAR (255),  -- full function name incl. db user
    @l_text                 VARCHAR (255)   -- output text
...
--                    AND sysstat & 0xf = 8
    ...
    IF (@@error <> 0)                    -- no error?
    BEGIN
        SELECT  @l_text = 'Error when deleting function ' + @ai_funcName + '.'
        PRINT @l_text                       -- print message
    END
GO
-- p_dropFunc
...
DECLARE
    ...
    -- local variables:
    @l_name                 VARCHAR (32),   -- name of savepoint
...
-- body:
    ...
    BEGIN
        SELECT  @l_id = @c_SAVEPOINT, @l_name = 's_' + @ai_name
 ...
 */
-- delete existing procedure:
EXEC p_dropProc 'p_rollbackTrans'
GO
-- create the new procedure:
 ...
 */
-- delete existing procedure:
EXEC p_dropProc 'p_binaryToHexString'
GO
 ...
 */
-- delete existing procedure:
EXEC p_dropProc 'p_intToHexString'
GO
