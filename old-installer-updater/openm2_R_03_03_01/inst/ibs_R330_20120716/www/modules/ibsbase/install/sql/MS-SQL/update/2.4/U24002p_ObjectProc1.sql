 ...
 *
 * @version     $Id: U24002p_ObjectProc1.sql,v 1.1 2005/01/27 01:20:30 klaus Exp $
 *
...
-- body:
    ...
    BEGIN
        ...
        SELECT  @l_tabTVersionId = t.tVersionId,
                ...
                @l_consistsOfId = c.id,
                @l_state = o.state
        FROM    ibs_Object o, ibs_ConsistsOf c, ibs_Tab t, ibs_ObjectDesc_01 d,
                ibs_TVersionProc p
        WHERE   o.oid = @ai_oid
...
-- body:
    ...
    SELECT  @l_coUserId = userId
    FROM    ibs_Checkout_01
    WHERE   oid = @ao_oid

    -- check if there occurred an error:
    EXEC @l_error = ibs_error.prepareErrorCount @@error, @@ROWCOUNT,
        'is the object checked out', @l_ePos OUTPUT, @l_count OUTPUT
    IF (@l_error <> 0)                  -- an error occurred?
        GOTO exception                  -- call common exception handler

    -- check if the current user is allowed to access the object:
    IF (@l_count > 0) AND (@l_coUserId <> @ai_userId)
    BEGIN
        -- set corresponding return value:
        SELECT @l_retValue = @c_INSUFFICIENT_RIGHTS
        GOTO exception                  -- call common exception handler
    ...
    SELECT  @l_containerId = containerId, @l_containerKind = containerKind,
            @l_tVersionId = tVersionId, @l_icon = icon, @l_owner = owner,
            @l_state = state
    FROM    ibs_Object
    WHERE   oid = @ao_oid
    ...
    BEGIN
        ...
        BEGIN
            ...
            BEGIN
                ...
                UPDATE  ibs_Object
                SET     flags = (flags | @c_INNEWS)
                WHERE   oid = @ao_oid

                -- check if there occurred an error:
                EXEC @l_error = ibs_error.prepareError @@error,
                    'set the showInNews bit', @l_ePos OUTPUT
                IF (@l_error <> 0)      -- an error occurred?
                    GOTO exception      -- call exception handler
            ...
            BEGIN
                ...
                UPDATE  ibs_Object
                SET     flags = (flags & (0x7FFFFFFF ^ @c_INNEWS))
                WHERE   oid = @ao_oid

                -- check if there occurred an error:
                EXEC @l_error = ibs_error.prepareError @@error,
                    'drop the showInNews bit', @l_ePos OUTPUT
                IF (@l_error <> 0)      -- an error occurred?
                    GOTO exception      -- call exception handler
...
AS
    ...
    DECLARE @posNoPath  POSNOPATH_VC       -- posnopath of object    
