/******************************************************************************
 * Task:        IBS-513 Create macedonian openm2 resource bundles
 *
 * Description: This file creates the locale mk_MK.
 *
 * Repeatable:  no
 *
 * @version     $Id: U300010u_createLocale_mk_MK.sql,v 1.1 2010/05/25 14:00:21 btatzmann Exp $
 *
...
--*****************************************************************************
    ...
    -- cleanup
    UPDATE ibs_object set state = 1 where name like 'mk_MK'
	...
	-- create locale 'mk_MK' for the domain within the locale
	-- container:
	EXEC @l_retVal = p_Locale_01$create @l_admin, @l_localOp, @c_TVLocale,
		'mk_MK', @l_cid_s, 1, 0, @c_NOOID_s, '',
		@l_localeOid_s OUTPUT
		
	IF (@l_retVal = @c_ALL_RIGHT)   -- locale mk_MK created?
	BEGIN
		EXEC p_stringToByte @l_localeOid_s, @l_localeOid OUTPUT
	
		-- create key mapper for locale mk_MK:
		EXEC p_KeyMapper$new @l_localeOid_s, 'locale_mk_MK', 'ibs_instobj'
		...
		EXEC @l_retVal = p_Locale_01$change @l_localeOid_s, @l_admin, 0,
			'mk_MK', N'#CONFVAR.ibsbase.validUntil#', '', 1, 'mk', 'MK', 0
	END -- if locale en_US created
	ELSE                            -- locale en_US not created
	BEGIN
		SELECT  @l_msg = 'cBData: Error when creating Locale mk_MK:' +
